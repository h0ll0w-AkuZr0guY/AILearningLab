...
- `ai` 必须写当前对话实际使用的产品与精确模型名称，优先取当前会话/接口元数据，原样保留大小写与版本；禁止从仓库历史、模板示例、上一批记录、自动化默认值或“最新模型”推断，也禁止把 WorkBuddy、Codex 或其他产品写成固定默认值。纯人工贡献写 `未使用 AI`；旧记录无法确认模型时写 `旧版本未记录`，禁止回填猜测。
- 若当前接口没有暴露可核验的产品或模型标识，必须暂停课程写作的 PR 发布门禁，使用 [`docs/PR_SIGNATURE_CONFIRMATION.md`](PR_SIGNATURE_CONFIRMATION.md) 向用户展示确认表，取得明确确认后才能写新日志、创建 draft PR 或标记 ready。用户确认只对当前批次有效。
- 自动化批次使用当前认证的 GitHub 用户或明确的课程 owner 作为 human，使用本次会话真实运行模型作为 ai。模型只负责部分工作时仍保留同一协作对，并在 summary 说明人工复核范围；每个批次都要重新确认，不能复用历史批次身份。
...
创建任何课程或课程规则 PR 前，AI 必须先展示 [`docs/PR_SIGNATURE_CONFIRMATION.md`](PR_SIGNATURE_CONFIRMATION.md) 的确认表，列出本批范围、human、产品、精确模型、证据来源和未修改范围。用户明确确认署名后才能创建 draft PR；PR URL 回写课程日志和恢复文档后，才可标记 ready。无法取得确认、接口身份不完整或用户改正字段时，批次停在本地验证阶段，不得提交错误署名。
