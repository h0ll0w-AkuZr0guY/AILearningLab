...
export const typescriptEventLoopGuides: Record<string, TopicGuide> = {
  'HTML event loop：task、microtask、render 与饥饿': {
    official: {
      ...
      note: 'HTML 的 processing model 定义了 task source、可运行 task 的选择、task 执行后的 microtask checkpoint、long task 记录与 rendering opportunity。它没有承诺每个 task 后都渲染，也没有把所有 task 合成一条全局 FIFO。'
    },
    source: {
      repo: 'v8/v8',
      ...
      code: `// 摘自 V8 main/src/execution/microtask-queue.cc。
...
int MicrotaskQueue::RunMicrotasks(Isolate* isolate) {
  ...
  return processed;
    ...
    chapters: [
      {
        title: '先分清 agent、event loop、task source 与 queue',
        ...
        code: `type TaskSource = "user" | "timer" | "message" | "network"
...
const taskQueues = new Map<TaskSource, Task[]>([
  ...
  ["network", []]
        ...
        language: 'typescript',
        takeaway: 'task source 内有次序，多个 source 之间由宿主调度；event loop 不能简化成唯一 FIFO。'
      },
      {
        title: '一次 event loop iteration 的真实顺序',
        ...
        code: `function runOneIteration(): void {
  ...
  maybeScheduleRenderingTask()
        ...
        language: 'typescript',
        takeaway: '一次 iteration 的稳定不变量是 task 完成后做 checkpoint；选择哪条 task queue 和是否渲染由宿主策略决定。'
      },
      {
        title: 'microtask checkpoint 会 drain-to-empty，并防止重入',
        ...
        code: `let performingCheckpoint = false
...
function performMicrotaskCheckpoint(): void {
  ...
  try {
    ...
    performingCheckpoint = false
        ...
        language: 'typescript',
        takeaway: 'checkpoint 是防重入的 drain-to-empty 生命周期边界，微任务中新建的微任务不会自动留到下一轮。'
      },
      {
        title: 'Promise reaction 与 queueMicrotask 同队列，错误通道不同',
...
setTimeout(() => {
  console.assert(order.join(",") === "sync,A,B,C")
        ...
        language: 'typescript',
        takeaway: '同为微任务只说明调度检查点相同；queueMicrotask 的未捕获异常与 Promise rejection 仍走不同协议。'
      },
      {
        title: 'rendering opportunity、rAF、layout 与 paint 不等于一个 JS 回调',
...
function scheduleVisualChange(change: () => void): void {
  ...
  requestAnimationFrame(() => {
    ...
    document.body.dataset.viewport = String(viewportWidth)
        ...
        language: 'typescript',
        takeaway: 'rAF 属于渲染更新协议；渲染机会可跳过，样式、布局、绘制也需要各自的时间预算和证据。'
      },
      {
        title: 'timer、MessageChannel、postTask 与真正的 yield',
        ...
        code: `async function yieldToHost(): Promise<void> {
  ...
  await new Promise<void>(resolve => {
    ...
    channel.port2.postMessage(undefined)
        ...
        language: 'typescript',
        takeaway: '微任务只重排当前 turn；CPU 切片要跨到未来 task，优先级、取消与时间预算也应进入协议。'
      },
      {
        title: '微任务饥饿、长任务与输入延迟如何被制造和测量',
        ...
        code: `async function processInSlices<T>(
  items: readonly T[],
  visit: (item: T) => void,
  budgetMs = 5,
  signal?: AbortSignal
  ...
  while (index < items.length) {
    ...
    if (index < items.length) await yieldToHost()
        ...
        language: 'typescript',
        takeaway: 'drain-to-empty 带来确定顺序，也允许饥饿；诊断要同时测执行耗时、排队延迟、帧与输入。'
      },
      {
        title: '浏览器、Worker 与 Node 要按宿主分别建模',
...
function schedulingDelay(trace: HostTrace): number | undefined {
  return trace.startedAt === undefined
    ...
    : trace.startedAt - trace.queuedAt
        ...
        language: 'typescript',
        takeaway: 'event loop 是宿主合同。浏览器、worker 和 Node 共享部分抽象，却有不同队列、阶段、渲染与生命周期。'
      }
    ],
    mechanisms: [
...
worker.onmessage = event => applyIndex(event.data)`,
    ...
    example: `type Source = "user" | "timer" | "message" | "render"
type LabTask = {
  id: number
  source: Source
  queuedAt: number
...
try {
  ...
  console.assert(error instanceof StarvationError)
    ...
    buildSteps: [
      {
        title: '积木 1：建立多个 task source',
        body: '为 user、timer、message、render 分别保存 FIFO；Task 记录 source、queuedAt、runnable 和 run。先测试同 source 顺序，再证明跨 source 由 chooseTask 策略决定。'
      },
      {
        title: '积木 2：实现 run-to-completion iteration',
        body: '选择一个 runnable task，设置 currentTask，执行后在 finally 清空。回调内部只能入队，不能直接让另一个 task 插入当前调用栈。'
      },
      {
        title: '积木 3：加入防重入 microtask checkpoint',
        body: '使用 performingCheckpoint 标志和 while drain；测试 A 中加入 C、队列已有 B 时得到 A→B→C，并让 nested checkpoint 成为 no-op。'
      },
      {
        title: '积木 4：加入渲染机会与 rAF',
        body: '用虚拟时钟在 frame deadline 到达时排 rendering task；rAF callback 只在该 task 中运行。加入“无视觉工作则跳过”测试，避免写成每轮强制 render。'
      },
      {
        title: '积木 5：记录排队延迟与 long task',
        body: '分别保存 queuedAt、startedAt、finishedAt，断言 scheduling delay 与 execution duration。让 task advance 60ms，观察 long-task 记录和后续输入延迟。'
      },
      {
        title: '积木 6：制造并识别微任务饥饿',
        body: '递归微任务会让 checkpoint 永不返回；教学实现加入 maxMicrotasksPerCheckpoint，并用专属 StarvationError 证明问题来自 drain-to-empty。'
      },
      {
        title: '积木 7：实现真实浏览器 yield 适配器',
        body: '优先 scheduler.yield，回退 MessageChannel；用 5ms 时间预算切分至少 10 万项计算。记录输入 handler 排队延迟，对照 await Promise.resolve 的失败版本。'
      },
      {
        title: '积木 8：用 DevTools 反证你的模型',
        body: '录制包含 timer、Promise、rAF、forced layout 和 Worker 的 Performance trace。逐项标注 task、microtask、Animation Frame、Layout、Paint，并写出教学调度器省略的 Chromium 优先级、合成线程和页面节流。'
      }
    ],
    selfCheckQuestion: '一个 timer task 中依次安排 queueMicrotask(A)、Promise.then(B) 和 requestAnimationFrame(R)，A 运行时再 queueMicrotask(C)。为什么常见结果是 A、B、C 先于 R？这个顺序中哪些部分是规范保证，哪些部分不能扩张为“每个 timer 后都必定渲染”？若 A 永远补入新的 A，setTimeout 与 R 会怎样，工程上应如何修复？',
    selfCheckAnswer: 'timer task 的同步 steps 完成后，HTML processing model 执行 microtask checkpoint。A 与 B 按入队次序进入微任务机制；A 运行时 C 被追加到仍含 B 的队尾，因此 checkpoint 的 drain-to-empty 得到 A→B→C。R 属于目标 Document 的 animation frame callbacks，要等窗口获得 rendering opportunity 并执行对应渲染更新，因而通常在该 checkpoint 之后。规范保证同一 checkpoint 排空及队列顺序，却允许用户代理跳过或合并 rendering opportunity，也允许在多个 task sources 间采用实现定义选择，所以不能承诺每个 timer 后绘一帧。若 A 每次都补入新 A，checkpoint 永远不空，event loop 无法选择下一 timer task，渲染任务也无法获得主线程，页面出现微任务饥饿。修复要给批处理设置每轮上限，把剩余工作通过 scheduler.yield、MessageChannel 或其他 future task 继续；重 CPU 工作可移到 Worker，并用 Performance trace 验证输入排队和帧恢复。'
  }
}
