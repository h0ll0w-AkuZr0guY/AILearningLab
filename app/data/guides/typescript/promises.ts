...
export const typescriptPromiseGuides: Record<string, TopicGuide> = {
  'Promise resolution、thenable assimilation 与 Job queue': {
    official: {
      ...
      note: 'Promise resolving function 用共享 [[AlreadyResolved]] cell 保证首次调用胜出；遇到 object resolution 时读取 then，若 callable 则创建 NewPromiseResolveThenableJob 并交给 HostEnqueuePromiseJob，稍后以新的 resolving functions 同化其最终状态。'
    },
    source: {
      repo: 'v8/v8',
      ...
      code: `// 摘自 V8 main/src/builtins/promise-resolve.tq。
...
transitioning builtin ResolvePromise(
  ...
  try {
    ...
    chapters: [
      {
        title: 'settled 描述状态，resolved 描述命运已锁定',
        ...
        code: `let release!: (value: number) => void
...
console.assert(await adopted === 7)`,
        language: 'typescript',
        takeaway: 'settled 是内部状态已终结；resolved 是第一次 resolving function 已决定命运，仍可能等待另一个 thenable。'
      },
      {
        title: 'resolve 与 reject 共享 alreadyResolved，第一次调用胜出',
...
console.assert(await thrownAfterResolve === 1)`,
        language: 'typescript',
        takeaway: 'resolve、reject 和调用后 throw 竞争同一个 alreadyResolved cell；只允许第一条完成路径影响 promise。'
      },
      {
        title: 'then getter 同步读取，then 调用异步排入 Job',
...
console.assert(
  events.join(",") === "get then,after resolve,call then"
        ...
        language: 'typescript',
        takeaway: 'Get then 是同步且只执行一次；调用 then 是排队的 Promise Job，二者必须分开画时间线。'
      },
      {
        title: 'self-resolution 与间接 thenable 环需要分别分析',
        ...
        code: `const base = Promise.resolve("ok")
...
console.assert(await Promise.resolve(repeated) === 1)`,
        language: 'typescript',
        takeaway: '规范显式拒绝直接 self-resolution；更长的 hostile thenable 环属于资源与信任边界问题。'
      },
      {
        title: 'then 为每个 reaction 创建新 capability 并吸收返回值',
...
console.assert(
  await Promise.reject("reason").finally(() => undefined)
    .catch(reason => reason) === "reason"
        ...
        language: 'typescript',
        takeaway: 'then 返回的是新 promise；handler 的返回或异常通过新 capability 的 resolve/reject 进入下一段状态机。'
      },
      {
        title: 'Promise Job 保留 Realm 与 execution context，host 决定检查点',
...
console.assert(order.join(",") === [
  ...
  "microtask C"
        ...
        language: 'typescript',
        takeaway: 'ECMAScript 定义 Promise Jobs 的因果关系，浏览器或 Node 定义 checkpoint；两层不能混成一句“宏任务微任务”。'
      },
      {
        title: 'rejection tracking 与错误处理存在时间窗口',
        ...
        code: `async function load(): Promise<string> {
  try {
...
try {
  ...
  console.assert(error.cause instanceof Error)
        ...
        language: 'typescript',
        takeaway: 'rejection tracking 是宿主级延迟观察；业务错误边界仍需明确传播、恢复、上下文与资源清理。'
      },
      {
        title: '工程 Promise API 还要补取消、背压与可观测性',
        ...
        code: `async function withTimeout<T>(
...
console.assert(value === 7)`,
        language: 'typescript',
        takeaway: 'Promise resolution 解决值和 thenable 的统一，取消、并发预算、资源释放与 trace 仍是上层 API 的责任。'
      }
    ],
    mechanisms: [
...
async function findUser(): Promise<Result<User, "missing">> {
  return { ok: false, error: "missing" }
      ...
      },
      {
        ...
        code: `const results = await mapConcurrent(
  inputs,
  ...
  input => processOne(input, signal)
    ...
    example: `type State = "pending" | "fulfilled" | "rejected"
type Reaction<T> = {
...
console.assert(events.join(",") === "get,executor end,call")`,
    buildSteps: [
      { title: '积木 1：建立三态与 reaction 队列', body: 'pending 保存两类 reactions；fulfill/reject 只能转换一次并把队列任务化。' },
      { title: '积木 2：共享 alreadyResolved cell', body: '让 executor 的 resolve/reject 捕获同一对象，覆盖 resolve→reject、reject→resolve 与调用后 throw。' },
      { title: '积木 3：实现 primitive 与 self-resolution', body: 'primitive 直接 fulfill；value===promise 以 TypeError reject，并写 then 返回自身的链式测试。' },
      { title: '积木 4：只读取一次 then', body: '用计数 getter证明 Get 恰好一次；getter throw 转 rejection，非 callable then 把 object 当值。' },
      { title: '积木 5：实现 thenable job', body: 'queueMicrotask 后才以 thenable 为 this 调 then，并为 job 创建第二组共享 resolve/reject 防多次调用。' },
      { title: '积木 6：实现 then 与 capability', body: '每次 then 返回新 MiniPromise；handler return/throw/thenable 分别验证下一链结果。' },
      { title: '积木 7：构造 job 时间线', body: '记录同步代码、then getter、thenable job、reaction job、queueMicrotask 与 timer，逐项解释谁负责入队。' },
      { title: '积木 8：补生产协议', body: '加入 AbortSignal、timeout 清理、并发窗口与 unhandled rejection 测试，明确 MiniPromise 未覆盖 Realm、species、debug hooks 和 host tracking。' }
    ],
    selfCheckQuestion: '执行 resolve(thenable) 时，then getter、then 函数、后续 reaction 分别在什么时候运行？如果 thenable 先 resolve(另一个 pending promise)，随后 reject 并抛错，外层 promise 当前是什么状态，最终由谁决定结果？',
    selfCheckAnswer: 'resolve 调用在当前同步栈中读取一次 then getter；getter 抛错立刻转成外层 rejection。若得到 callable then，只创建 PromiseResolveThenableJob 并入队，then 函数要到 Promise Job 运行时才以 thenable 为 this 调用。then 内第一次调用 resolve 赢得 job 自己的 alreadyResolved cell；它若解析到另一个 pending promise，外层 promise 的命运已锁定但内部状态仍可保持 pending。随后 reject 与 throw 都因 alreadyResolved 已 true 被忽略。另一个 pending promise 最终 fulfill 或 reject 时，才沿采纳链决定外层 settlement，并把已登记的 reactions 排成后续 PromiseReactionJobs。'
  }
}
