...
export const typescriptClassGuides: Record<string, TopicGuide> = {
  'prototype、new、class fields 与 private brand': {
    official: {
      ...
      note: 'ClassDefinitionEvaluation 建立 constructor、prototype、extends 两条原型链、私有名称环境与字段初始化器；真正构造实例时，OrdinaryConstruct 和 InitializeInstanceElements 再依次安装私有方法、私有字段与公开字段。'
    },
    source: {
      repo: 'v8/v8',
      ...
      code: `// 摘自 V8 main/src/interpreter/bytecode-generator.cc。
...
void BytecodeGenerator::BuildInstanceMemberInitialization(
  ...
  builder()
    ...
    chapters: [
      {
        title: '对象只有原型链，class 提供声明与初始化协议',
...
console.assert(User.prototype.constructor === User)`,
        language: 'typescript',
        takeaway: 'class 把 constructor、prototype 属性描述符与实例初始化打包；运行时方法查找仍沿普通对象原型链完成。'
      },
      {
        title: 'extends 同时连接实例侧与静态侧两条原型链',
...
console.assert(Object.getPrototypeOf(NullBase.prototype) === null)`,
        language: 'typescript',
        takeaway: '继承不是一根链：prototype 对象链服务实例成员，constructor 对象链服务 static 成员。'
      },
      {
        title: 'new、NewTarget 与 prototype 选择组成实例诞生协议',
...
console.assert(Object.getPrototypeOf(reflected) === Child.prototype)`,
        language: 'typescript',
        takeaway: 'Target 决定执行哪段构造逻辑，NewTarget 决定默认原型与最派生构造身份。'
      },
      {
        title: '公开字段使用 DefineField，时序不同于 constructor 赋值',
...
console.assert(events.join(",") === "body:1,body:1")`,
        language: 'typescript',
        takeaway: '字段定义的核心是 DefineField 与初始化时序；它不只是 constructor 顶部的一条 this.x 赋值。'
      },
      {
        title: '私有名称按声明身份区分，private brand 证明合法接收者',
...
try {
  ...
  console.assert(error instanceof TypeError)
        ...
        language: 'typescript',
        takeaway: 'private brand 是来源证明：对象必须经过当前 class 定义对应的初始化，伪造同名公开属性没有作用。'
      },
      {
        title: '派生构造器在 super 后安装自己的 brand 与字段',
...
console.assert(order.join(" > ") === [
  ...
  "child body"
        ...
        language: 'typescript',
        takeaway: '继承层级各自初始化自己的字段与品牌；super 返回点是派生 this 从未初始化转为可用的关口。'
      },
      {
        title: 'V8 用隐藏 initializer 与内部 brand 保持规范顺序',
        ...
        code: `// 可观察 shape 的实验应固定构造路径，再逐步引入差异。
class StablePoint {
  x = 0
  y = 0
...
console.assert(Object.keys(points[0]).join(",") === "x,y")`,
        language: 'typescript',
        takeaway: '源码阅读要分清可观察语义与引擎表示；shape、IC 和 brand 优化都必须保持 class 初始化协议。'
      },
      {
        title: 'TypeScript private、#private 与 emit target 具有不同保证',
...
console.assert(runtime.reveal() === "brand checked")`,
        language: 'typescript',
        takeaway: '先明确需要静态设计约束、运行时品牌检查还是安全边界，再选择 TS private、#private、closure 或 WeakMap。'
      }
    ],
    mechanisms: [
    ...
    variants: [
      {
        title: 'prototype method + public state',
...
try {
  ...
  console.assert(error instanceof TypeError)
    ...
    buildSteps: [
      { title: '积木 1：实现普通原型查找', body: '对象保存 prototype 与 own property map；Get 从 receiver 开始沿链查找，并用缺失属性测试走到 null。' },
      { title: '积木 2：建立 constructor/prototype 双链', body: '创建 Parent、Child 和各自 prototype object，分别验证静态继承链与实例继承链，避免把两者画成同一根箭头。' },
      { title: '积木 3：实现 construct(target, newTarget)', body: '从 newTarget.prototype 分配实例，执行 target body，加入 object return 替换与 primitive return 忽略测试。' },
      { title: '积木 4：实现 DefineField', body: '字段直接写 own property map，不调用原型 setter；对照普通 Set 写一个能观察二者差异的父 setter 反例。' },
      { title: '积木 5：按继承顺序初始化字段', body: '基类字段→基类 body→派生字段→派生 body；让每步写入 timeline，失败时精确指出尚未出现的状态。' },
      { title: '积木 6：用 symbol identity 实现 Private Name', body: '每次 defineClass 创建唯一 symbol；两个同名 #x 使用不同 symbol，并证明普通字符串键无法通过 privateGet。' },
      { title: '积木 7：加入 brand 与重复初始化保护', body: '实例安装 class brand；借用方法到 imposter 时抛 TypeError，同一 class 在同一对象重复初始化也必须失败。' },
      { title: '积木 8：对照 V8 快慢路径', body: '把 class context 可直接定位视为快速路径，把嵌套 arrow/eval 的 super 视为慢路径；记录教学版未覆盖的 context depth、IC feedback 与 abrupt completion。' }
    ],
    selfCheckQuestion: '为什么 class Child extends Parent 会形成两条原型链？请进一步解释：父类 constructor 中调用 this.render() 时，为什么能分派到 Child.prototype.render，却可能读不到 Child 的公开字段和 #private 字段？',
    selfCheckAnswer: 'Child 自身的 [[Prototype]] 指向 Parent，用于继承 static 成员；Child.prototype 的 [[Prototype]] 指向 Parent.prototype，用于实例方法查找。new Child() 以 Child.prototype 作为最终实例原型，所以父 constructor 里的 this.render() 对同一实例执行 [[Get]] 时，会先在实例和 Child.prototype 找到 override。可此时只完成了父层的 InitializeInstanceElements：派生层字段与 private brand 要等 super() 返回到 Child constructor、this 被绑定后才安装。因此 override 虽已能通过原型链被找到，Child 的 public field 仍是缺失/undefined，访问尚未安装的 #private 则会触发品牌检查 TypeError。这也是基类构造器不应调用可覆盖方法的重要原因。'
  }
}
