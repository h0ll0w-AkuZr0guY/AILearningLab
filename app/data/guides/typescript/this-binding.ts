...
export const typescriptThisBindingGuides: Record<string, TopicGuide> = {
  'this、arrow、call/apply/bind、new 与 super': {
    official: {
      ...
      note: 'EvaluateCall 先判断被调用表达式是否产生 property Reference：若是，则用 GetThisValue(ref) 得到 receiver；否则 thisValue 为 undefined。后续 OrdinaryCallBindThis、Bound Function [[Call]]、[[Construct]] 与 GetSuperBase 分别处理普通函数、绑定函数、构造调用和 super。'
    },
    source: {
      repo: 'v8/v8',
      ...
      code: `// 摘自 V8 main/src/builtins/x64/builtins-x64.cc。
...
void Builtins::Generate_ConstructBoundFunction(MacroAssembler* masm) {
  ...
  __ TailCallBuiltin(Builtin::kConstruct);
    ...
    chapters: [
      {
        title: 'this 的第一现场是 Reference，不是函数对象',
...
console.assert((0, account.read).call(another) === 11)`,
        language: 'typescript',
        takeaway: '先判断调用前是否仍有 property Reference；同一个函数值可以因不同 Reference 得到不同 receiver。'
      },
      {
        title: 'OrdinaryCallBindThis 区分 lexical、strict 与 global 三种模式',
        kicker: '02 · THIS MODE',
        paragraphs: [
        ...
        code: `function strictProbe(this: unknown) {
  "use strict"
  return this
}
...
console.assert(boxed === "[object Number]")`,
        language: 'typescript',
        takeaway: 'receiver 先由调用表达式产生，普通函数再按自身 [[ThisMode]] 决定保留、全局替换或装箱。'
      },
      {
        title: '箭头函数跳过 this 绑定，并沿词法环境取值',
...
console.assert(
  ...
  Counter.prototype.prototypeMethod
        ...
        language: 'typescript',
        takeaway: '箭头固定的是词法 this 解析路径；它解决回调 receiver 丢失，同时放弃动态 receiver、构造能力和共享 prototype identity。'
      },
      {
        title: 'call、apply 与 bind 操作的是不同时间点',
...
console.assert(boundA.length === 1)`,
        language: 'typescript',
        takeaway: 'call/apply 立即提供一次调用的 receiver；bind 创建可重复调用的新对象，并永久保存第一层绑定的 receiver 与参数前缀。'
      },
      {
        title: 'new 进入 [[Construct]]，thisArgument 已不再是主角',
        ...
        code: `function Base(this: { kind?: string }) {
  this.kind = "base"
}
...
console.assert(value.marker === "alternate")`,
        language: 'typescript',
        takeaway: '构造调用围绕 [[Construct]] 与 NewTarget 组织；“创建空对象后 call 一次”只能模拟最简单 base constructor 的表面结果。'
      },
      {
        title: 'bound constructor 忽略 boundThis，却保留参数前缀',
...
console.assert((BoundPoint as unknown as typeof Point).category === undefined)`,
        language: 'typescript',
        takeaway: 'bound function 的 [[Call]] 与 [[Construct]] 是两条协议：前者消费 boundThis，后者只消费 boundArgs 并修正 NewTarget。'
      },
      {
        title: 'super 用 HomeObject 找起点，用当前 this 做 Receiver',
...
console.assert(borrowed.call({ name: "borrowed" }) === "parent:borrowed")`,
        language: 'typescript',
        takeaway: 'super 需要两个坐标：HomeObject 固定查找起点，当前 this 作为 Receiver 贯穿父 getter、setter 与方法。'
      },
      {
        title: 'TypeScript 的 this 参数把调用契约写进类型系统',
...
console.assert(write("ready") === "[app] ready")`,
        language: 'typescript',
        takeaway: '显式 this 参数能把动态调用约束前移，但不会改变 JavaScript 的 Reference、[[Call]] 或 [[Construct]] 语义。'
      }
    ],
    mechanisms: [
    ...
    variants: [
      {
        title: 'prototype method + 调用点保留 receiver',
...
button.addEventListener("click", () => service.run())`,
  ...
  submit() {}
      ...
      },
      {
        ...
        code: `type Context = { traceId: string }
...
console.assert((holder as { x?: number }).x === 8)`,
    buildSteps: [
      {
        title: '积木 1：把调用目标建模为 Reference',
        body: '定义 base 与 value，分别执行 obj.method() 和 detached()；断言只有 property Reference 把 base 交给 call。'
      },
      {
        title: '积木 2：实现三种 this mode',
        body: '在教学 Function Environment 中加入 lexical/strict/global；覆盖 undefined、null、primitive 与跨 realm 的测试。'
      },
      {
        title: '积木 3：实现 BoundCallable [[Call]]',
        body: '保存 target、boundThis、boundArgs；调用时忽略外部 receiver，按 boundArgs + args 顺序递归调用 target。'
      },
      {
        title: '积木 4：加入重复 bind 与 identity 测试',
        body: '对 bound function 再 bind，证明内层 boundThis 胜出、两层参数依次前置，并验证每次 bind 产生不同函数 identity。'
      },
      {
        title: '积木 5：实现 base constructor',
        body: '新增 construct(args, newTarget)，从 newTarget.prototype 建对象，调用函数体，并实现返回对象替换、primitive 忽略规则。'
      },
      {
        title: '积木 6：实现 BoundCallable [[Construct]]',
        body: '复用 boundArgs、忽略 boundThis；当 newTarget 等于 bound wrapper 时转成 target，并用自定义 NewTarget 验证原型转发。'
      },
      {
        title: '积木 7：分离 HomeObject 与 Receiver',
        body: '为 method 保存 homeObject，superGet 从 homeObject 的原型查找 descriptor，却把当前 this 作为 getter Receiver；用 borrowed method 验证两坐标。'
      },
      {
        title: '积木 8：工程化验收',
        body: '为事件订阅缓存唯一 bound callback，加入 dispose；为动态 receiver API 写显式 this 参数，并记录真实 V8 builtin 与教学实现的差异。'
      }
    ],
    selfCheckQuestion: '为什么 const B = A.bind(fakeThis, 1) 后，B.call(other, 2) 使用 fakeThis，而 new B(2) 却创建新实例？同时说明 newTarget 在 new B 与 Reflect.construct(B, [2], C) 两种调用中的转发差异。',
    selfCheckAnswer: 'B 是 Bound Function exotic object。普通 [[Call]] 读取 [[BoundThis]]=fakeThis，把 [[BoundArguments]]=[1] 与 [2] 拼接后以 fakeThis 调用 A；外部 call 传入的 other 被丢弃。若 A 可构造，B 的 [[Construct]] 只拼接参数并 Construct(A, [1,2], newTarget)，完全不读取 boundThis。new B(2) 的初始 newTarget 是 B，规范会把它修正为 A，所以默认从 A.prototype 创建实例；Reflect.construct(B, [2], C) 的 newTarget 是显式 C，与 B 不同，必须原样转发，因此实例原型来自 C.prototype。V8 的 CallBoundFunction 覆盖 receiver，而 ConstructBoundFunction 只压入 bound arguments 并条件修正 newTarget，正好对应这两条路径。'
  }
}
