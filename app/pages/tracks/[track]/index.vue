...
const sourceNeedle = (scope?: string) => {
  if (!scope) return ''
  if (/仓库根目录/.test(scope) && /tests/i.test(scope)) return 'tests'
  return scope.split(/\s+与\s+|[、，,]/)[0]?.trim() || ''
}

const docsNeedle = (url: string, title: string) => {
  try {
    const parsed = new URL(url)
    return decodeURIComponent(parsed.hash.slice(1)) || title
  } catch {
    return title
  }
}

const graphLink = (lesson: Lesson, mode: 'source' | 'docs') => {
  const detail = getLessonDetail(track.value!, lesson)
  const module = track.value!.modules.find(item => item.order === lesson.moduleOrder)
  const needle = mode === 'source'
    ? (detail.curated ? detail.source.symbol || detail.source.file : sourceNeedle(module?.sourceScope))
    : (detail.curated ? docsNeedle(detail.official.url, detail.official.title) : '')
  return {
    path: '/source-map',
    query: {
  ...
  <div class="track-page">
    <main class="shell">
      <section class="page-hero">
        ...
        <div class="track-meta"><span>{{ track?.lessons.length }} 节专题课</span><span>{{ modules.length }} 个能力模块</span><span>{{ curatedCount }}/{{ track?.lessons.length }} 已深度精写</span><a :href="track?.source" target="_blank" rel="noreferrer">上游源码 ↗</a><a :href="track?.docs" target="_blank" rel="noreferrer">官方文档 ↗</a><NuxtLink :to="{ path: '/source-map', query: { track: track?.id, mode: 'source' } }">资料图谱 ↗</NuxtLink><a href="#interviewer">面试官实战 ↓</a></div>
      </section>
