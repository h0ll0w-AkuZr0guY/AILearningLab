...
11. 每次修改本课都在文末“更新日志”顶部追加一条记录。human 与 ai 永远成对出现：
    ...
    - ai 使用当前对话实际协作产品与精确模型，优先原样读取会话/接口元数据；禁止从仓库历史、模板示例、自动化配置或其他批次复制产品名/模型名。纯人工贡献写“未使用 AI”。若接口未暴露可核验身份，必须先按 docs/PR_SIGNATURE_CONFIRMATION.md 向用户确认，确认前不得创建或标记 PR。
