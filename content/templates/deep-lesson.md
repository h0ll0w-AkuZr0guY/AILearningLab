...
11. 每次修改本课都在文末“更新日志”顶部追加一条记录。human 与 ai 永远成对出现：
    ...
    - ai 使用实际协作产品与模型，必须写本次实际运行的模型：产品统一写 WorkBuddy，模型名以当前运行环境为准（例如本自动化配置 model_id 为 deepseek-v4-flash 时写“WorkBuddy · DeepSeek-V4-Flash”）；纯人工贡献写“未使用 AI”，禁止猜测、虚构或套用其它批次的产品名。
