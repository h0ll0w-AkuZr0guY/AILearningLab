...
函数对象的 [[Environment]] 保存创建函数时的 Environment Record；调用时 NewFunctionEnvironment 把它作为 [[OuterEnv]]。for 头部使用 let/const 时，CreatePerIterationEnvironment 会为指定名称建立新的 binding，并从上一轮 binding 复制当前值，因此每轮闭包可以观察不同 binding。
...
```cpp
// 摘自 V8 src/runtime/runtime-scopes.cc；函数签名与 Factory 调用为真实源码，
...
```typescript
function makeCell(initial: number) {
  let value = initial
  return {
    read: () => value,
...
分析闭包时画 binding identity：哪些函数共享同一 binding，哪些只是收到某次 GetValue 的结果。

### 调用环境通过函数的 [[Environment]] 接回创建环境
...
闭包的外层环境在函数创建时确定；每次执行创建语句是否产生新环境，决定状态是共享还是隔离。

### for let 每轮复制值到新 binding
...
for-let 的关键是每轮 binding identity 不同；“let 是块级作用域”本身还不足以解释三个回调为何得到三个值。
...
异步不是闭包的另一套语义。先确定何时创建函数和 binding，再把回调调用时间放进事件时间线。
...
```typescript
function noEscape(items: number[]) {
...
规范决定结果，逃逸分析决定表示。优化闭包前先证明函数 identity 或捕获环境真的成为瓶颈。

### 从 retained path 处理闭包泄漏
...
闭包生命周期应从“谁持有函数、何时释放”设计。dispose、取消和容量边界比盲目避免闭包更有效。
...
### 积木 1：实现共享 binding cell

让两个 closure 通过同一 Binding 对象读写，证明捕获不是数值快照。

### 积木 2：保存创建环境

函数对象显式保存 environment，调用时从该环境解析名称；从另一个动态环境调用也不得改变结果。

### 积木 3：实现 per-iteration 环境复制

为指定名称建立新 Binding，并复制上一轮 GetBindingValue；用三个 reader 验证 0/1/2。

### 积木 4：加入 var 共享反例

让三次创建都保存同一个函数环境 binding，循环后读取 3/3/3，并用参数 binding 修复。

### 积木 5：做 retained path 实验

监听器捕获 10MB buffer，heap snapshot 找到 listener→function→context→buffer；dispose 后重复快照验证释放。

## 自检

### 问题

for (let i = 0; i < 3; i++) 创建的三个闭包为什么分别读到 0、1、2？为什么说“let 是块级作用域”仍不是完整答案？

### 站内答案

for 头部的 lexical declaration 让 i 进入 perIterationBindings。ForBodyEvaluation 在进入循环并在每轮 body 后调用 CreatePerIterationEnvironment：为 i 创建新的 mutable binding，把上一轮 binding 当前值复制进去，然后 increment 在新环境中更新它。当轮创建的闭包把当轮 LexicalEnvironment 保存到 [[Environment]]，所以三个函数最终沿不同环境读取三个不同 binding。仅说“块级作用域”只说明 i 不在函数级 var 环境，无法解释同一个循环 body 的不同迭代为什么仍不共享一个块 binding。
