...
Environment Record 是规范用来关联 Identifier 与 binding 的抽象记录，具有 [[OuterEnv]] 链和 Create/Initialize/Get/Set 等方法；execution context 保存当前代码求值状态、LexicalEnvironment、VariableEnvironment、PrivateEnvironment、Realm 与活动函数。两者都是规范模型，不要求与引擎栈帧或某个 JavaScript 对象一一对应。
...
```cpp
// 摘自 V8 src/interpreter/bytecode-generator.cc，保留真实 location 分派与 TDZ 检查。
...
不要画“代码搬家”。画一张表：声明在实例化阶段创建什么 binding、初值是什么、执行到 initializer 时又发生什么。

### Environment Record 是名称解析协议
...
shadowing 在 binding 创建时已经生效。TDZ 读取命中的是内层未初始化 binding，而不是“找不到变量”。

### Create、Initialize、Get、Set 构成 binding 生命周期
...
```typescript
console.assert(typeof trulyMissing === "undefined")
...
TDZ 是 binding 状态检查，不是时间计时器。控制流若永远未执行初始化，binding 就一直不能读。

### var、function 与 global object 的边界
...
```typescript
function varScope(flag: boolean) {
  if (flag) {
...
“全局、函数、块”还不够；先确定 Script/Module/CommonJS、strict/sloppy 与 declaration kind，再推导 binding 落点。

### Execution Context 不等于操作系统线程或固定栈帧
...
await 保存的是可恢复求值状态，恢复由 job 调度；它不是另开线程，也不是让原同步调用栈一直挂着。

### V8 把 binding 分配到 register、Context 与 global/module slot
...
规范 environment 解释语义，V8 variable location 解释成本。把两层对齐，但不要把寄存器/Context 当作跨版本承诺。

### TypeScript 只做静态近似，运行时仍由 ECMAScript 初始化
...
checker 能降低漏初始化概率，无法替代运行时 binding/field 顺序。复杂生命周期应建模成状态，而不是用 ! 抹掉证据。
...
```typescript
type Client = Readonly<{ endpoint: URL; token: string }>
...
用独立 sentinel 区分 uninitialized 与 undefined，实现 create、initialize、get、set 和 mutable/immutable 错误。

### 积木 2：加入 environment chain

实现 HasBinding/resolve 与 outer 链，写外层值、内层同名 TDZ、初始化后 shadow 三段测试。

### 积木 3：模拟声明实例化

输入 var/function/let/const 声明表：var 初始化 undefined，function 初始化 callable，lexical 保持 hole；再按 statement 顺序求 initializer。

### 积木 4：拆分 Lexical/Variable Environment

进入 block 只更新 LexicalEnvironment；var 写入 VariableEnvironment。用 if/for/catch 验证 binding 落点。

### 积木 5：加入 closure capture

函数对象保存创建时 environment；调用建立 Function Environment，证明两个 closure 共享同一 captured binding。

### 积木 6：对照 V8 bytecode

用 node --print-bytecode 或 d8 观察 local、captured context、global、module 与 TDZ 示例，映射到 BuildVariableLoad 分支。

### 积木 7：建立初始化顺序回归矩阵

覆盖 typeof undeclared/TDZ、class extends/static field、默认参数访问 body var、ESM 循环依赖与 async 挂起后的 binding。

## 自检

### 问题

为什么“let 不会提升”是一个有害简化？请用外层同名变量、typeof 和 V8 hole check 三个证据说明。

### 站内答案

let binding 在进入其 block 的 BlockDeclarationInstantiation 时已经创建，所以从 block 起就遮蔽外层同名 binding；只是它保持 uninitialized，直到执行声明的 BindingInitialization。因名称已解析到内层 binding，声明前读取不会退回外层；typeof 也必须对该 Reference 执行 GetBindingValue，因此抛 ReferenceError，而真正不可解析名称的 typeof 才返回 "undefined"。V8 会为可能提前读取的 local/context/module load 生成 ThrowReferenceErrorIfHole，以内部 the_hole 区分未初始化与合法 undefined。这些都说明 binding 被提前建立了，“不提升”无法解释真实状态。
