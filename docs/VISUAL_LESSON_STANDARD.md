...
通用渲染器只提供视觉语法，不替课程作叙事判断。仅把标题、颜色和步骤文字换掉，仍然属于同一个通用实验。出现以下任一信号时，应设计专属组件：

- 学习者需要直接操作课程中的真实变量，例如 stride、storage offset、队列、barrier 或 UI props。
- 同一时刻必须比较两条以上信息通道，例如业务值与 trace、逻辑张量与物理 Storage。
- 节点完成顺序、分支条件或失败注入会改变后续状态，线性“下一步”无法表达。
- 课程结论依赖可计算结果，组件应现场算出地址、shape、队列或状态差异。

专属组件不以代码量评判。它至少应包含“可操作输入、机制状态、证据读数”三层，并让读者能把读数回指正文公式、源码分支或示例断言。视觉鲜明来自信息层次、对比和反馈，不来自持续漂浮、发光或装饰粒子。
...
- **Python**：首轮深化内容没有按当前长课与视觉证据规范完成，暂不作为视觉覆盖对象。后续逐模块重写时再按真实学习障碍选择 graph、state、flow、tensor 或 playground，不继承旧批量视觉结论。

这些只是首选，不替代课题分析。一节课可以有两个视觉块，但每个块必须解决不同学习障碍。

截至 2026-07-31，非 Python 的 53 节 curated 课程均已建立伴随索引：TypeScript 13、LangGraph 10、Transformer 10、PyTorch 15、LangChain 5。Python 的 102 节保留正文供后续重构参考，但视觉索引已撤回，避免给旧批量内容制造“已经按新规范验收”的错觉。后续新增课程仍须逐课判断，当前覆盖不能演变成强制配额。
...
4. 先在纸面列出 3–5 个离散状态，每个状态绑定一个证据；再判断通用组件能否表达课程特有的变量、并行关系与可计算读数。
...
10. Pages 部署后随机抽查至少一节通用视觉、一节专属交互和一张图片课程。若本批没有对应媒介，不为凑验收类型临时增加。
