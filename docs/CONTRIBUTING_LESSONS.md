...
`human` 始终记录发起或负责贡献的人类，`ai` 始终记录本次实际协作模型（产品统一写 WorkBuddy，模型名以实际运行环境为准）。纯人工写 `未使用 AI`；模型版本无法从可靠证据确认时写 `旧版本未记录`。外部贡献者使用自己的 AI 提交 PR 时，署名应是该贡献者与其 AI 的协作对，不能只写仓库所有者或只写模型。
