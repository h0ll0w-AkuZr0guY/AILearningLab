...
human: "@your-name"
ai: "<本次会话真实披露的产品> · <本次会话真实披露的模型>"
summary: "区分 pending writes 与外部副作用窗口，增加崩溃恢复实验。"
pr: "https://github.com/owner/repository/pull/123"
commit: "abcdef1"
```

`human` 始终记录发起或负责贡献的人类，`ai` 始终记录当前对话实际协作的产品与精确模型（优先读取会话/接口元数据，禁止从历史记录、模板或自动化默认值推断）。纯人工写 `未使用 AI`；模型版本无法从可靠证据确认时，必须先按 [`docs/PR_SIGNATURE_CONFIRMATION.md`](PR_SIGNATURE_CONFIRMATION.md) 请求用户确认，不能猜测或创建 PR。外部贡献者使用自己的 AI 提交 PR 时，署名应是该贡献者与其 AI 的协作对，不能只写仓库所有者或只写模型。

课程 PR 创建前必须把确认表中的 human、ai、证据来源、批次范围和未修改范围发给用户；用户明确确认后才允许创建 draft PR。下一批或换会话必须重新确认，历史记录只追加纠正，不得覆盖。
