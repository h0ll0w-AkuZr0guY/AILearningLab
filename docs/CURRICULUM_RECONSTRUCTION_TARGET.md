...
## TypeScript 尾批本地准备：typescript-01-14（已完成署名确认，待发布）
...
- 本地准备：已在分支 `agent/curriculum-typescript-typescript-01-14-to-typescript-01-14` 认领 catalog，新增课程正文与 `examples/typescript/typescript-01-14.mjs`；示例正常/失败断言通过，`corepack pnpm run curriculum:audit` 与 `git diff --check` 通过。署名确认后已建立 `visuals/typescript-01-14.md`，登记 `flow`、静止默认、5 步、固定 `chapter:2` placement，并将 catalog 状态改为 `curated`，等待发布前完整门禁。
- 视觉决策：正文之后仍难以追踪“同一 package name 如何依次经过条件解析、格式桥接和两张 cache 身份表”，计划使用静止默认 `flow` 视觉，5 步展示 request kind → exports target → CJS/ESM facade → namespace/async failure → cache owner；不使用图片或专属组件。正式建立索引后复验单步、暂停、重置、键盘、窄屏与 reduced motion。
...
- 发布状态：PR、合并 commit、Pages run 和线上 URL 尚未产生；接下来执行冻结安装、审计、示例、生成、敏感信息与构建产物检查、浏览器验收，再创建 draft PR 并回写 PR URL。下一 pending 为 LangChain `langchain-01-11`、`langchain-01-12`；未认领的 TypeScript draft 模块课题不纳入本批。
