...
## LangChain 动态深化批次：langchain-01-06 至 langchain-01-10（本地待署名）
...
- 本地验证：`CI=true corepack pnpm install --frozen-lockfile` 成功；`corepack pnpm run curriculum:audit` 成功，LangChain `5/12 → 10/12`，全站 `120/134 → 125/134`，视觉 `120/120 → 125/125`；五门 Python 示例均分别执行了正常与失败断言；`corepack pnpm generate` 成功并预渲染 298 条路由；`git diff --check` 成功。生成期间发现并修复了未创建 PR 的占位链接导致的伪 404 路由，当前日志暂不含 `pr` 字段，避免把门禁占位符渲染为链接。
...
- 分支与发布：已从当前 `origin/main` 建立本地分支 `agent/curriculum-langchain-langchain-01-06-to-langchain-01-10`，其 merge-base 为 `56e3a6fe3f751cc418cf475ab214170d54183229`；PR URL、squash merge commit、远端分支删除、Pages run、线上 URL 与线上验收均为待署名确认后的事项。确认前不得推送、创建 draft PR 或改写日志中的 `ai`。
